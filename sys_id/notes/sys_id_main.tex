...
\framebox[\linewidth]{
    \begin{minipage}{\dimexpr\linewidth-2\fboxsep-2\fboxrule\relax}
        \textbf{Definition:} A matrix $\textbf{M}$ is positive semi-definite when $\forall \textbf{v} \neq 0$,
        \begin{equation}
            \textbf{v}^\top \textbf{M} \textbf{v} \geq 0
        \end{equation}
    \end{minipage}
}
...
\subsection{November 4th}
\subsubsection{Gradient Descent}
As we pursue our optimal solution for a problem,
\begin{equation}
    \textbf{x}^* = \arg \min_{\textbf{x}} f(\textbf{x})
\end{equation}
gradient descent is a powerful tool to bring us closer to the optimal solution where the gradient is equal to zero.
We pick an initial point $\textbf{x}_0$ and use a step-size weighted pursuit opposite of the gradient to iteratively drive toward the optimal solution at $\nabla f= 0$.
\begin{equation}
    \textbf{x}^{k+1} = \textbf{x}^k - \alpha^k \nabla_{\textbf{x}} f(\textbf{x})
\end{equation}
We also develop and implement a stopping criterion with the goal of stopping once $\textbf{x}^ \approx \textbf{x}^*$.
Below are some examples of stopping criteria.
\begin{itemize}
    \item We can stop once the change is small enough $f(\textbf{x}^{k+1}) - f(\textbf{x}^k) \leq \varepsilon_f$
    \item We can stop once the gradient of the function is small enough to be considered approximately zero $ \| \nabla f(\textbf{x}^k) \|  \leq \varepsilon_g$.
    \item We can also look at the change in the arguments to evaluate if the change is small enough where we have an approximately optimal solution $ \| \textbf{x}^{k+1} - \textbf{x}^k \| \leq \varepsilon_x$. This is effectively the same as the previous one.
    \item We can also stop when we don't have enough time with a number of iterations $l$ and $l_{max}$. $l \leq l_{max}$
\end{itemize}

Gradient descent is a first-order solution method.
It requires the first derivative in order to work.
There exist zeroth-order and second-order methods (as well as a general $n$-th order).
\begin{equation}
    \begin{cases}
        \text{Zeroth-Order} & f(x) \\
        \text{First-Order} & \nabla f(x) \\
        \text{Second-Order} & H(f(x))
    \end{cases}
\end{equation}
\subsubsection{Zeroth Order Methods}
A zeroth order method only utilises information from the function $f(x)$.
One of these methods is called \textbf{random search} or \textbf{random walk}, in which a random direction and step is pursued and evaluated.
We can evaluate if we increased or decreased at all and see where we achieved the lowest solution.
\begin{equation}
    \textbf{x}^{k+1} = \textbf{x}^k + \alpha^{k} \textbf{rand}
\end{equation}
Using the $\alpha$ term, we can approximate simulated annealing by varying the step size to balance exploration and expolitation.
With a high step size, we can jump out of local minima.

Another zeroth order method uses the approximate gradient by measuring the distance between points $\delta = \textbf{x}^{k+1} - \textbf{x}^k$.
\begin{equation}
    \textbf{x}^{k+1} = \textbf{x}^k - \alpha^k \tilde{\nabla}_{\textbf{x}} f(\textbf{x})
\end{equation}

\subsubsection{Second Order Methods}
The second order methods take advantage of the parabola with the same slope and curvature at the point $f(\textbf{x})$.
We can then jump to the bottom of the parabola and drive down to the optimal solution of the original function.
Below is the taylor series that we can use to flesh out how to utilise the second order derivative to solve the function.
\begin{equation}
    f(\textbf{x}) \approx f(\hat{\textbf{x}}) + \nabla f (\hat{\textbf{x}})^\top (\textbf{x} - \hat{\textbf{x}}) + \frac{1}{2} (\textbf{x} - \hat{\textbf{x}})^\top \textbf{H}(f(\hat{\textbf{x}})) (\textbf{x} - \hat{\textbf{x}})
\end{equation}
We want to get the minimum of $f(\textbf{x})$ and can do so by getting the minimum of the right hand side
\begin{equation}
    \arg \min_{\textbf{x}} \hat{f}(\textbf{x}) = \arg \min_{\textbf{x}} \{ \frac{1}{2} (\textbf{x} - \hat{\textbf{x}})^\top \textbf{H}(f(\hat{\textbf{x}})) (\textbf{x} - \hat{\textbf{x}}) + \nabla f(\hat{\textbf{x}})^\top (\textbf{x} - \hat{\textbf{x}}) \}
\end{equation}

\begin{equation}
    (\textbf{x} - \hat{\textbf{x}}) = - \textbf{H} (\hat{\textbf{x}})^{-1} \nabla_{\textbf{x}} f(\hat{\textbf{x}})
\end{equation}

\begin{equation}
    \textbf{x}^* = \hat{\textbf{x}} - \textbf{H} (\hat{\textbf{x}})^{-1} \nabla_{\textbf{x}} f(\hat{\textbf{x}})
\end{equation}
We are able to arrive to an iterative algorithm using this
\begin{equation}
    \textbf{x}^{k+1} = \textbf{x}^k - \alpha^k \textbf{H} (\textbf{x}^k)^{-1} \nabla_{\textbf{x}} f(\textbf{x}^k)
\end{equation}
Clearly, this only works when the hessian is positive definite $\textbf{H} \succ 0$.
In the event that we do not have access to a positive definite hessian, we resort to the gradient descent without the hessian.
This Newton's Method is effectively weighting the gradient descent by the hessian to improve the speed of convergence.

In the event that we have a quadratic equation as our original problem, this clearly converges to the optimal solution in one step when $\alpha = 1$.
A general version would be to replace $\textbf{H}$ with $\textbf{H} + \textbf{M}$ where $\textbf{M}$ is a term that adds the identity matrix to ensure we use gradient descent if the hessian is not positive definite.

\subsection{November 10th}

\begin{equation}
    \hat{\theta}^{l+1} = \hat{\theta}^l - \alpha^l [H(\hat{\theta} + M^l)]^{-1} \nabla_{\hat{\theta}} J_N (\hat{\theta}^l)
\end{equation}
where
\begin{equation}
    J_N(\hat{\theta}) = \frac{1}{N} \sum_{r=1}^{N} (y(r) - \hat{y}(r | \hat{\theta}))^2
\end{equation}

of which we can represent the error as $e(r|\hat{\theta}) = y(r) - \hat{y}(r | \hat{\theta}) $.

We can figure out the gradient of this sum of errors
\begin{gather}
    \frac{d J_N (\hat{\theta})}{d \hat{\theta}} = \frac{1}{N} \sum_{r=1}^{N} \frac{d e^2}{d \hat{\theta}} \\
    = \frac{1}{N}   \sum_{r=1}^{N} 2 e \frac{d e}{d \hat{\theta}} 
\end{gather}

we can do the same for the hessian to get
\begin{equation}
    \frac{d^2 J}{d \hat{\theta}^2 } = \frac{2}{N} \sum_{r=1}^{N} \frac{d e}{d \hat{\theta}}^\top \frac{d e}{d \hat{\theta}} + e \frac{d^2 e}{d \hat{\theta}^2}
\end{equation}

The second term should approximate zero as we get closer to no error.
If our selected $\hat{\theta}$ is good, then we have a good approximation and our error $e$ approaches zero.
We can create this algorithm that is kind of an adaptive version of newtons method where at the beginning iterations, we use gradient descent and as we get closer, we introduce more of the second derivative to get closer to the actual solution.

\begin{equation}
   \hat{\theta}^{l+1} = \hat{\theta}^l - \alpha^l [\frac{2}{N} \sum_{r=1}^{N} (\frac{d e(r|\hat{\theta})}{d \hat{\theta}}^\top \frac{d e(r|\hat{\theta})}{d \hat{\theta}}) + \delta^l \textbf{I} ]^{-1} [\frac{2}{N} \sum_{r=1}^{N} e(r|\hat{\theta}) \frac{d e(r|\hat{\theta})}{d \hat{\theta} } ]^\top
\end{equation}

As an application example, lets consider an ARMAX model with a collected dataset with entries $r$.
\begin{equation}
    \hat{y}(r | \hat{\theta}) = [1-\frac{A(z)}{D(z)}] y(r) + \frac{B(z)}{D(z)} u(r)
\end{equation}
therefore to get the error $e$, we can plug this in
\begin{gather}
    e(r|\hat{\theta}^l) = y(r) - \hat{y}(r|\hat{\theta}^l) \\
    = y(r) - [1-\frac{A(z)}{D(z)}] y(r) + \frac{B(z)}{D(z)} u(r) \\
    = \frac{A(z|\hat{\theta}^l)}{D(z|\hat{\theta}^l)} y(r) - \frac{B(z|\hat{\theta}^l)}{D(z|\hat{\theta}^l)}
\end{gather}

\subsection{November 24th}
With two assumptions we can say that the system is perfectly modeled by the system:
\begin{itemize}
    \item $S = M(\theta)$
    \item $\hat{\theta} = \theta$
\end{itemize}
If these two assumptions are true, then the error corresponds to a white noise following the noise in the system.
\begin{equation}
    e(k | \hat{\theta}) = y(k) - \hat{y} (k| \hat{\theta}) = n(k)
\end{equation}
where $n(k)$ follows $N(0,\lambda^2)$.
We can also conclude that if the error does not follow a white noise normal distribution, then one or both of the assumptions are failed.
Either, the model does not fit the system appropriately, or the parameters estimated do not appropriately characterize the system.

We can think about a whiteness test to determine whether a sequence of data points come from "white noise".
We can conclude with a certain level of confidence or accuracy that a sequence is white or not.
The whiteness test is a measure of how well the model adheres to the assumptions, and therefore it can be used to compare different models' performances.

In order to get an informative enough dataset, we need to have an input that is persistently exciting.
Our input should be variable enough to capture enough variance in the data to learn from it.
With an infinite amount of data (or a set large enough to capture most meaningful variance), overfitting is more difficult to achieve.
...
where $\phi$ is the regressor
\begin{align}
  \phi(k) = 
  \begin{bmatrix}
     - y(k-1) \\ \vdots \\ -y(k-m) \\ u(k) \\ \vdots \\ u(k-n)
  \end{bmatrix}
\end{align}
...
We can then create a general function $\gamma: \mathbb{R}^{m + n +1} \to \mathbb{R}$ that maps the parameters and regressors into the output.
\begin{equation}
    \hat{y} (k | \hat{\theta}) = \hat{\gamma} (\phi(k), \hat{\theta})
\end{equation}
where for ARX, $\hat{\gamma} = \phi^\top \hat{\theta}$.
With an ARX model, we can say that the first assumption holds and that the system itself is ARX if 
\begin{equation}
    y(k) = \phi(k)^\top \theta + n(k)
\end{equation}.

This generalized function $\gamma$ is a non linear difference equation.
This can create a N-ARX class of models which is the non-linear ARX models.
\begin{equation}
    y(k) = \gamma(\phi(k)) + n(k)
\end{equation}
Our goal is then to find the optimal function $\gamma^*$ to minimize some functional cost $J$.
\begin{equation}
    \gamma^* = \arg \min_{\hat{\gamma}} J_N (\gamma) = \frac{1}{N} \sum_{r=1}^{N} [y(r) - \hat{\gamma}(\phi(k))]^2
\end{equation}

\subsection{December 15th}
For asymptotical stability in continuous time we require that the characteristic matrix $F-HG$ has strictly negative real parts to the eigenvalues.
For discrete time, the absolute value of all eigenvalues should be less than one.
\begin{equation}
    \textbf{e}(k+1) = (F-HG) \textbf{e}(k)
\end{equation}

with this description of a system,
\begin{gather}
    \textbf{z}(k+1) = \hat{F} \textbf{z}(k) + \hat{W} \textbf{u}(k) + H \textbf{y} (k) \\
    \hat{\textbf{x}}(k) = \textbf{z} (k)
\end{gather}
it is asymptotical observer if 
\begin{itemize}
    \item $\hat{W} = W$
    \item $\hat{F} = F - HG$ 
    \item $H$ s.t. $F-HG$ is asymptotically stable
\end{itemize}
For the third condition, we can choose $H$ to adhere to this constraint if $S$ is completely observable.
OR, if all parts of $S$ that are not observable are asymptotically stable.

We can break up any system into the observable parts and the non-observable parts.
The input influences both the observable part and the non-observable, but the observable part has a one-way effect on the non-observable part.
The non-observable part does not have a direct effect on the output but can cause the system to explode if it is asymptotically unstable.

Assume we have $\hat{F}, \hat{W}, H$ that adhere to the previous three constraints
\begin{gather}
    \textbf{z}(k+1) = (F-HG)\textbf{z}(k) + W \textbf{u}(k) + H \textbf{y}(k) \\
    = F \textbf{z} + W \textbf{u} + H(\textbf{y} - G \textbf{z})
\end{gather}
this is an asymptotical observer with $\hat{\textbf{x}}(k) = \textbf{z}(k)$.
In the event that $z$ is the same as the state $x$, then the term $\textbf{y} - G \textbf{z}$ approaches zero.

Bringing it back to the recursive ARX model
\begin{equation}
    \hat{\theta}^*(k) = \hat{\theta}^* (k-1) + \gamma (k) [y(k) - \hat{y} (k | \hat{\theta}^* (k-1))]
\end{equation}

we can augment the state with parameters that we do not know and create a $X(k)$ from $x(k)$ and $\theta$.

We can also have time-varying matrices for our system and observer.
\begin{gather}
    \textbf{x}(k+1) = F(k) \textbf{x}(k) + W(k) \textbf{u}(k) + \xi(k) \\
    \textbf{y}(k) = G(k) \textbf{x}(k) + \eta(k)
\end{gather}
where $\xi, \eta$ are white noise random variables with time-varying covariances $Q(k), R(k)$ respectively.

\begin{itemize}
    \item $\hat{\textbf{x}}(k | k)$ is an estimate of the state $\textbf{x}(k)$ based on the current information set $I(k)$.
    \item $\hat{\textbf{x}}(k | k - 1)$ is a prediction of the state $\textbf{x}(k)$ based on the previous information set $I(k-1)$
\end{itemize}

\begin{gather}
    \hat{\textbf{x}} (k | k) = E[\textbf{x} (k) | I(k)] \\
    \Sigma(k | k) = E[ (\textbf{x}(k) - \hat{\textbf{x}}(k | k))(\textbf{x}(k) - \hat{\textbf{x}}(k | k))^\top | I(k)]
\end{gather}
