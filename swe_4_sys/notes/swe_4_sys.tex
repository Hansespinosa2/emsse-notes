...
The use cases of a system define the objectives of that system.

\section{Wednesday 03/25/2026}
We tend to start a project with a use case, and define it by what it must accomplish.

Everything done in SysML follows a hierarchy.

There are \textbf{block definition diagrams} which define replicable blocks and their relationships with each other, and \textbf{internal block diagrams} which define the parts and components inside the block that define the interactions with the other blocks.

The difference between an activity and an action is that an activity is a composite of actions, whereas an action cannot be drilled down into or split into further actions.

\section{Thursday 03/26/2026}
The difference between a library and a stereotype is that the library is limited within our model, and does not travel outside those limits, whereas a stereotype can be applied to other models.
Generally, the difference is not too big, and a library can generally be the default, and converted into a stereotype when needed.
